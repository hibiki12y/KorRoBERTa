...
fileHandler = logging.FileHandler('./roberta_pretraining.log')
...
def main():
    ...
    if args.local_rank == -1 or args.no_cuda:
        ...
        torch.distributed.init_process_group(backend='nccl',
                                             ...
                                             rank=args.local_rank + args.start_rank,
    ...
    if args.local_rank != -1:
        num_train_optimization_steps = math.ceil(num_train_optimization_steps / torch.distributed.get_world_size())

    # Prepare model
    config = BertConfig.from_json_file(os.path.join(args.bert_model, args.bert_config_file))
    logger.info('{}'.format(config))
    ###############################################
    # Load Model
    if args.load_model:
        load_model_name = os.path.join(args.output_dir, args.load_model_name)
        model = BertForPreTraining.from_pretrained(args.bert_model, state_dict=torch.load(load_model_name)["state_dict"])
    else:
    ...
    if args.load_model:
        ...
        loaded_states = torch.load(os.path.join(args.output_dir, args.load_model_name), map_location=device)
        optimizer.load_state_dict(loaded_states["optimizer"])

        regex = re.compile(r'\d+epoch')
        epoch0 = int(regex.findall(args.load_model_name)[-1].replace('epoch', ''))
        logger.info('extract {} -> epoch0 : {}'.format(args.load_model_name, epoch0))
    ...
    for epoch in range(epoch0, args.epochs):
        ...
        with tqdm(total=len(train_dataloader), desc='training..') as pbar:
            for step, batch in enumerate(train_dataloader):
                ...
                if (step + 1) % 50 == 0:
                    pbar.set_description("Epoch = {}, global_step = {}, loss = {:.5f}".format(epoch, global_step+1, mean_loss))
                    logger.info("Epoch = {}, global_step = {}, loss = {:.5f}".format(epoch, global_step + 1, mean_loss))
                ...
                if (step + 1) % args.save_step == 0 :
                    if args.local_rank == -1 or args.local_rank == 0 :
                        ...
                        output_model_file = os.path.join(args.output_dir, args.model_name + "_{}step".format(global_step))
                        model_to_save = model.module if hasattr(model, 'module') else model
                        state = {"state_dict": model_to_save.state_dict(), "optimizer": optimizer.state_dict()}
                        torch.save(state, output_model_file)

        if args.local_rank == -1 or args.local_rank == 0:
            logger.info("** ** * Saving {} - epoch model ** ** * ".format(epoch))
            output_model_file = os.path.join(args.output_dir, args.model_name + "_{}epoch".format(epoch + 1))
            model_to_save = model.module if hasattr(model, 'module') else model
            state = {"state_dict": model_to_save.state_dict(), "optimizer": optimizer.state_dict()}
            torch.save(state, output_model_file)
